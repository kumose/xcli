...
int main(int argc, char **argv) {
    ...
    xcli::App *start = app.add_subcommand("start", "A great subcommand");
    xcli::App *stop = app.add_subcommand("stop", "Do you really want to stop?");
    app.require_subcommand();  // 1 or more

    std::string file;
    start->add_option("-f,--file", file, "File name");

    xcli::Option *s = stop->add_flag("-c,--count", "Counter");
