...
class ConfigJSON : public xcli::Config {
  public:
    std::string to_config(const xcli::App *app, bool default_also, bool, std::string) const override {

        json j;

        for(const xcli::Option *opt : app->get_options({})) {
        ...
        for(const xcli::App *subcom : app->get_subcommands({}))
            j[subcom->get_name()] = json(to_config(subcom, default_also, false, ""));

        return j.dump(4);
    }

    std::vector<xcli::ConfigItem> from_config(std::istream &input) const override {
        json j;
        input >> j;
        return _from_config(j);
    }

    std::vector<xcli::ConfigItem>
    _from_config(json j, std::string name = "", std::vector<std::string> prefix = {}) const {
        std::vector<xcli::ConfigItem> results;
        ...
        if(j.is_object()) {
            ...
            xcli::ConfigItem &res = results.back();
            res.name = name;
            res.parents = prefix;
            if(j.is_boolean()) {
                ...
                throw xcli::ConversionError("Failed to convert " + name);
            }
        } else {
            throw xcli::ConversionError("You must make all top level values objects in json!");
        }
