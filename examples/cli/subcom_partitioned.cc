...
int main(int argc, char **argv) {
    xcli::AutoTimer give_me_a_name("This is a timer");

    xcli::App app("K3Pi goofit fitter");

    xcli::App_p impOpt = std::make_shared<xcli::App>("Important");
    std::string file;
    xcli::Option *opt = impOpt->add_option("-f,--file,file", file, "File name")->required();

    int count{0};
    xcli::Option *copt = impOpt->add_flag("-c,--count", count, "Counter")->required();

    xcli::App_p otherOpt = std::make_shared<xcli::App>("Other");
