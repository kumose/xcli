...
TEST_CASE_METHOD(TApp, "doubleVectorFunctionFail", "[optiontype]") {
    ...
    CHECK_THROWS_AS(app[vstring]->results(res), xcli::ConversionError);
...
TEST_CASE_METHOD(TApp, "doubleVectorFunctionRunCallbackOnDefault", "[optiontype]") {
    ...
    CHECK_THROWS_AS(opt->default_val("this is a string"), xcli::ConversionError);
    ...
    opt->check(xcli::Number);
    opt->run_callback_for_default(false);
    CHECK_THROWS_AS(opt->default_val("this is a string"), xcli::ValidationError);
#endif
}
