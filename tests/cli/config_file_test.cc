...
TEST_CASE("StringBased: convert_arg_for_ini", "[config]") {
    ...
    CHECK("\"\"" == xcli::detail::convert_arg_for_ini(std::string{}));

    CHECK("true" == xcli::detail::convert_arg_for_ini("true"));

    CHECK("nan" == xcli::detail::convert_arg_for_ini("nan"));

    CHECK("\"happy hippo\"" == xcli::detail::convert_arg_for_ini("happy hippo"));

    CHECK("47" == xcli::detail::convert_arg_for_ini("47"));

    CHECK("47.365225" == xcli::detail::convert_arg_for_ini("47.365225"));

    CHECK("+3.28e-25" == xcli::detail::convert_arg_for_ini("+3.28e-25"));
    CHECK("-22E14" == xcli::detail::convert_arg_for_ini("-22E14"));

    CHECK("'a'" == xcli::detail::convert_arg_for_ini("a"));

    CHECK("'\\'" == xcli::detail::convert_arg_for_ini("\\"));
    ...
    CHECK("'B\"(\\x00)\"'" == xcli::detail::convert_arg_for_ini(tstring1));
    // hex
    CHECK("0x5461FAED" == xcli::detail::convert_arg_for_ini("0x5461FAED"));
    // hex fail
    CHECK("\"0x5461FAEG\"" == xcli::detail::convert_arg_for_ini("0x5461FAEG"));

    // octal
    CHECK("0o546123567" == xcli::detail::convert_arg_for_ini("0o546123567"));
    // octal fail
    CHECK("\"0o546123587\"" == xcli::detail::convert_arg_for_ini("0o546123587"));

    // binary
    CHECK("0b01101110010" == xcli::detail::convert_arg_for_ini("0b01101110010"));
    // binary fail
    CHECK("\"0b01102110010\"" == xcli::detail::convert_arg_for_ini("0b01102110010"));
}
...
TEST_CASE("StringBased: IniJoin", "[config]") {
    ...
    CHECK(result == xcli::detail::ini_join(items, ' ', '\0', '\0'));

    result = R"(["one", "two", "three four"])";

    CHECK(result == xcli::detail::ini_join(items));
...
TEST_CASE("StringBased: First", "[config]") {
    ...
    std::vector<xcli::ConfigItem> output = xcli::ConfigINI().from_config(ofile);
...
TEST_CASE("StringBased: EmptySectionSegment", "[config]") {
    ...
    std::vector<xcli::ConfigItem> output;
    CHECK_NOTHROW(output = xcli::ConfigINI().from_config(ofile));
...
TEST_CASE("StringBased: TomlMultiLineStringError", "[config]") {
    ...
    CHECK_THROWS(xcli::ConfigINI().from_config(ofile));
}
...
bool checkSections(const std::vector<xcli::ConfigItem> &output) {
...
TEST_CASE("StringBased: file_error", "[config]") {
    CHECK_THROWS_AS(xcli::ConfigINI().from_file("nonexist_file"), xcli::FileError);
}
...
TEST_CASE_METHOD(TApp, "IniIgnoreRemainingOption", "[config]") {
    ...
    app.set_config("--config", tmpini);
    app.allow_config_extras(xcli::config_extras_mode::ignore);
...
class EvilConfig : public xcli::Config {
  public:
    EvilConfig() = default;
    std::string to_config(const xcli::App *, bool, bool, std::string) const override { throw xcli::FileError("evil"); }

    std::vector<xcli::ConfigItem> from_config(std::istream &) const override { throw xcli::FileError("evil"); }
};
...
TEST_CASE_METHOD(TApp, "IniRequiredbadConfigurator", "[config]") {
    ...
    std::shared_ptr<xcli::Config> evilptr = evil;
    app.config_formatter(evil);
    int two{0};
    app.add_option("--two", two);
    REQUIRE_THROWS_AS(run(), xcli::FileError);

    REQUIRE_THROWS_AS(evilptr->to_config(&app, xcli::ConfigOutputMode::Active, true, ""), xcli::FileError);
}
...
TEST_CASE_METHOD(TApp, "MultiConfig_takelast", "[config]") {
    ...
    app.set_config("--config")->multi_option_policy(xcli::MultiOptionPolicy::TakeLast)->expected(1, 3);
...
TEST_CASE_METHOD(TApp, "MultiConfig_takeAll", "[config]") {
    ...
    app.set_config("--config")->multi_option_policy(xcli::MultiOptionPolicy::TakeAll);
...
TEST_CASE_METHOD(TApp, "MultiConfig_single", "[config]") {
    ...
    app.set_config("--config")->multi_option_policy(xcli::MultiOptionPolicy::TakeLast);
...
TEST_CASE_METHOD(TApp, "IniRequiredNotFound", "[config]") {
    ...
    CHECK_THROWS_AS(run(), xcli::FileError);
}
...
TEST_CASE_METHOD(TApp, "IniInlineComment", "[config]") {
    ...
    app.config_formatter(std::make_shared<xcli::ConfigINI>());
...
TEST_CASE_METHOD(TApp, "IniJoinEmbeddedDelimiterRoundTrip", "[config]") {
    ...
    auto *opt = app.add_option("--vals", vals)->delimiter(',')->multi_option_policy(xcli::MultiOptionPolicy::Join);
...
TEST_CASE_METHOD(TApp, "TOMLVectordirect", "[config]") {
    ...
    app.config_formatter(std::make_shared<xcli::ConfigTOML>());
...
TEST_CASE_METHOD(TApp, "IniFailure", "[config]") {
    ...
    CHECK_THROWS_AS(run(), xcli::ConfigError);
}
...
TEST_CASE_METHOD(TApp, "IniNotConfigurable", "[config]") {
    ...
    CHECK_THROWS_AS(run(), xcli::ConfigError);
    app.allow_config_extras(xcli::config_extras_mode::ignore_all);
    CHECK_NOTHROW(run());
}
...
TEST_CASE_METHOD(TApp, "IniNoSubFailure", "[config]") {
    ...
    app.set_config("--config", tmpini);
    app.allow_config_extras(xcli::config_extras_mode::error);
    {
        std::ofstream out{tmpini};
        out << "[other]" << '\n';
        out << "val=1" << '\n';
...
TEST_CASE_METHOD(TApp, "IniFlagConvertFailure", "[config]") {
    ...
    CHECK_THROWS_AS(opt->results(result), xcli::ConversionError);
    std::string res;
    opt->results(res);
    CHECK("moobook" == res);
}
...
TEST_CASE_METHOD(TApp, "IniDefaultPath", "[config]") {
    ...
    int key{0};
    app.add_option("--flag,-f", key);
    app.set_config("--config", "TestIniTmp.ini")->transform(xcli::FileOnDefaultPath("../"));
...
TEST_CASE_METHOD(TApp, "IniMultipleDefaultPath", "[config]") {
    ...
    auto *cfgOption = app.set_config("--config", "doesnotexist.ini")
                          ->transform(xcli::FileOnDefaultPath("../"))
                          ->transform(xcli::FileOnDefaultPath("../other", false));
...
TEST_CASE_METHOD(TApp, "IniMultipleDefaultPathAlternate", "[config]") {
    ...
    app.add_option("--flag,-f", key);
    auto *cfgOption = app.set_config("--config", "doesnotexist.ini")
                          ->transform(xcli::FileOnDefaultPath("../other") | xcli::FileOnDefaultPath("../"));
...
TEST_CASE_METHOD(TApp, "IniEnvironmental", "[config]") {
    ...
    int key{0};
    app.add_option("key", key)->envname("XCLI_TEST_ENV_KEY_TMP");
    app.set_config("--config", tmpini);

    {
        std::ofstream out{tmpini};
        out << "XCLI_TEST_ENV_KEY_TMP=3" << '\n';
    }
...
TEST_CASE_METHOD(TApp, "TomlOutputEnvironmental", "[config]") {
    ...
    int val{1};
    app.add_option(std::string{}, val)->envname("XCLI_TEST_ENV_TMP");

    run();

    CHECK(val == 2);
    std::string str = app.config_to_str();
    CHECK(str == "XCLI_TEST_ENV_TMP=2\n");
...
TEST_CASE_METHOD(TApp, "ConfigOutputVectorCustom", "[config]") {
    ...
    std::vector<int> v;
    app.add_option("--vector", v);
    auto V = std::make_shared<xcli::ConfigBase>();
    ...
    str = V->to_config(&app, xcli::ConfigOutputMode::AllDefaults, false, "");
    CHECK(str == "vector:{1; 2; 3}\n");
}
...
TEST_CASE_METHOD(TApp, "TomlOutputSet", "[config]") {
    ...
    int v{0};
    app.add_option("--simple", v)->check(xcli::IsMember({1, 2, 3}));
...
TEST_CASE_METHOD(TApp, "ConfigWriteDefaultActiveSubcommands", "[config]") {
    ...
    auto *create = app.add_subcommand("create");
    std::string create_path{"create-path"};
    create->add_option("--create-path", create_path)->capture_default_str()->check(xcli::NonexistentPath);

    auto *load = app.add_subcommand("load");
    std::string load_path{"load-path"};
    load->add_option("--load-path", load_path)->capture_default_str()->check(xcli::ExistingPath);

    args = {"create"};
    run();

    std::string activeConfig = app.config_to_str(xcli::ConfigOutputMode::ActiveSubcommandDefaults, false);
...
TEST_CASE_METHOD(TApp, "CrashTest", "[config]") {
    ...
    app.set_config("--conf")->check(xcli::ExistingFile);
