...
TEST_CASE_METHOD(TApp, "OptionGroupInvalidNames", "[optiongroup]") {
    CHECK_THROWS_AS(app.add_option_group("clusters\ncluster2", "description"), xcli::IncorrectConstruction);

    std::string groupName("group1");
    groupName += '\0';
    groupName.append("group2");

    CHECK_THROWS_AS(app.add_option_group(groupName), xcli::IncorrectConstruction);
}
...
TEST_CASE_METHOD(TApp, "BasicOptionGroupMinMovedAsGroup", "[optiongroup]") {
    ...
    CHECK_THROWS_AS(ogroup->add_options(opt1), xcli::OptionNotFound);
    args = {"--option", "9"};
    CHECK_THROWS_AS(run(), xcli::RequiredError);
...
TEST_CASE_METHOD(TApp, "BasicOptionGroupAddFailures", "[optiongroup]") {
    ...
    auto *ogroup = app.add_option_group("clusters");
    CHECK_THROWS_AS(ogroup->add_options(app.get_config_ptr()), xcli::OptionAlreadyAdded);
    CHECK_THROWS_AS(ogroup->add_options(app.get_help_ptr()), xcli::OptionAlreadyAdded);

    auto *sub = app.add_subcommand("sub", "subcommand");
    auto *opt2 = sub->add_option("--option2", val2);

    CHECK_THROWS_AS(ogroup->add_option(opt2), xcli::OptionNotFound);

    CHECK_THROWS_AS(ogroup->add_options(nullptr), xcli::OptionNotFound);

    ogroup->add_option(opt1);

    auto *opt3 = app.add_option("--test1", res);

    CHECK_THROWS_AS(ogroup->add_option(opt3), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "BasicOptionGroupScrewedUpMove", "[optiongroup]") {
    ...
    CHECK_THROWS_AS(ogroup2->add_options(opt1, opt2), xcli::OptionNotFound);

    xcli::Option_group EmptyGroup("description", "new group", nullptr);

    CHECK_THROWS_AS(EmptyGroup.add_option(opt2), xcli::OptionNotFound);
    CHECK_THROWS_AS(app._move_option(opt2, ogroup2), xcli::OptionNotFound);
}

TEST_CASE_METHOD(TApp, "InvalidOptions", "[optiongroup]") {
    auto *ogroup = app.add_option_group("clusters");
    xcli::Option *opt = nullptr;
    CHECK_THROWS_AS(ogroup->excludes(opt), xcli::OptionNotFound);
    xcli::App *app_p = nullptr;
    CHECK_THROWS_AS(ogroup->excludes(app_p), xcli::OptionNotFound);
    CHECK_THROWS_AS(ogroup->excludes(ogroup), xcli::OptionNotFound);
    CHECK_THROWS_AS(ogroup->add_option(opt), xcli::OptionNotFound);
}
...
struct ManyGroups : public TApp {
    ...
    xcli::Option_group *main{nullptr};
    xcli::Option_group *g1{nullptr};
    xcli::Option_group *g2{nullptr};
    xcli::Option_group *g3{nullptr};
    std::string name1{};
    std::string name2{};
    std::string name3{};
    std::string val1{};
...
TEST_CASE_METHOD(ManyGroups, "getGroup", "[optiongroup]") {
    ...
    CHECK(mn == main);
    CHECK_THROWS_AS(app.get_option_group("notfound"), xcli::OptionNotFound);
}
...
TEST_CASE_METHOD(TApp, "ExistingSubcommandMatch", "[optiongroup]") {
    auto sshared = std::make_shared<xcli::Option_group>("documenting the subcommand", "sub1g", nullptr);
...
TEST_CASE_METHOD(ManyGroups, "OptionFind", "[optiongroup]") {
    ...
    CHECK_THROWS_AS(g1->get_option("--notfound"), xcli::OptionNotFound);
    auto const *g1_const = g1;
    const auto *opt_name_const = g1_const->get_option("--base");
    CHECK(opt_name_const == opt_main);
    CHECK_THROWS_AS(g1_const->get_option("--notfound"), xcli::OptionNotFound);
}
