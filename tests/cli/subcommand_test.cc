...
TEST_CASE_METHOD(TApp, "BasicSubcommands", "[subcom]") {
    ...
    CHECK_THROWS_AS(app.get_subcommand("sub3"), xcli::OptionNotFound);
    ...
    try {
        run();
    } catch(const xcli::ExtrasError &e) {
...
TEST_CASE_METHOD(TApp, "Callbacks", "[subcom]") {
    auto *sub1 = app.add_subcommand("sub1");
    sub1->callback([]() { throw xcli::Success(); });
    auto *sub2 = app.add_subcommand("sub2");
    bool val{false};
...
TEST_CASE_METHOD(TApp, "RuntimeErrorInCallback", "[subcom]") {
    auto *sub1 = app.add_subcommand("sub1");
    sub1->callback([]() { throw xcli::RuntimeError(); });
    auto *sub2 = app.add_subcommand("sub2");
    sub2->callback([]() { throw xcli::RuntimeError(2); });

    args = {"sub1"};
    CHECK_THROWS_AS(run(), xcli::RuntimeError);

    args = {"sub1"};
    try {
        run();
    } catch(const xcli::RuntimeError &e) {
...
TEST_CASE_METHOD(TApp, "BadSubcommandSearch", "[subcom]") {
    ...
    CHECK_THROWS_AS(app.get_subcommand(two), xcli::OptionNotFound);
    CHECK_THROWS_AS(app.get_subcommand_ptr(two), xcli::OptionNotFound);
}
...
TEST_CASE_METHOD(TApp, "InheritHelpAllFlag", "[subcom]") {
    ...
    auto *subc = app.add_subcommand("subc");
    auto help_opt_list = subc->get_options([](const xcli::Option *opt) { return opt->get_name() == "--help-all"; });
...
TEST_CASE_METHOD(TApp, "RequiredPosInSubcommand", "[subcom]") {
    ...
    xcli::App *fooApp = app.add_subcommand("foo", "Foo a bar");
    fooApp->add_option("bar", bar, "A bar to foo")->required();

    xcli::App *bazApp = app.add_subcommand("baz", "Baz a bar");
...
TEST_CASE_METHOD(TApp, "invalidSubcommandName", "[subcom]") {
    ...
    } catch(const xcli::IncorrectConstruction &e) {
        gotError = true;
...
struct SubcommandProgram : public TApp {
    ...
    xcli::App *start{nullptr};
    xcli::App *stop{nullptr};
...
TEST_CASE_METHOD(TApp, "SubcomInheritCaseCheck", "[subcom]") {
    ...
    CHECK(app.get_subcommands().empty());
    CHECK(app.get_subcommands({}).size() == 2u);
    CHECK(app.get_subcommands([](const xcli::App *s) { return s->get_name() == "sub1"; }).size() == 1u);

    // check the const version of get_subcommands
    const auto &app_const = app;
    CHECK(app_const.get_subcommands([](const xcli::App *s) { return s->get_name() == "sub1"; }).size() == 1u);
...
TEST_CASE_METHOD(TApp, "SubcomInheritUnderscoreCheck", "[subcom]") {
    ...
    CHECK(app.get_subcommands().empty());
    CHECK(app.get_subcommands({}).size() == 2u);
    CHECK(app.get_subcommands([](const xcli::App *s) { return s->get_name() == "sub_option1"; }).size() == 1u);
...
TEST_CASE_METHOD(SubcommandProgram, "Subcommand HelpAllOrder", "[subcom]") {
    ...
    args = {"--help-all"};
    CHECK_THROWS_AS(run(), xcli::CallForAllHelp);
...
TEST_CASE_METHOD(SubcommandProgram, "Subcommand Callbacks", "[subcom]") {
    ...
    start->callback([]() { throw xcli::Success(); });
...
struct ManySubcommands : public TApp {
    ...
    xcli::App *sub1{nullptr};
    xcli::App *sub2{nullptr};
    xcli::App *sub3{nullptr};
    xcli::App *sub4{nullptr};
...
TEST_CASE_METHOD(ManySubcommands, "manyIndexQuery", "[subcom]") {
    ...
    CHECK(sub3 == s3);
    CHECK(sub4 == s4);
    CHECK_THROWS_AS(app.get_subcommand(4), xcli::OptionNotFound);
...
TEST_CASE_METHOD(ManySubcommands, "manyIndexQueryPtr", "[subcom]") {
    ...
    CHECK(sub3 == s3.get());
    CHECK(sub4 == s4.get());
    CHECK_THROWS_AS(app.get_subcommand_ptr(4), xcli::OptionNotFound);
}
...
TEST_CASE_METHOD(ManySubcommands, "manyIndexQueryPtrByName", "[subcom]") {
    ...
    CHECK(sub3 == s3.get());
    CHECK(sub4 == s4.get());
    CHECK_THROWS_AS(app.get_subcommand_ptr("sub5"), xcli::OptionNotFound);
}
...
TEST_CASE_METHOD(ManySubcommands, "SubcommandOptionExclusion", "[subcom]") {
    ...
    try {
        run();
    } catch(const xcli::ExcludesError &ee) {
...
TEST_CASE_METHOD(ManySubcommands, "SubcommandNeedsFail", "[subcom]") {
    ...
    sub1->needs(opt);
    CHECK_THROWS_AS(sub1->needs((xcli::Option *)nullptr), xcli::OptionNotFound);
    CHECK_THROWS_AS(sub1->needs((xcli::App *)nullptr), xcli::OptionNotFound);
    CHECK_THROWS_AS(sub1->needs(sub1), xcli::OptionNotFound);
...
TEST_CASE_METHOD(ManySubcommands, "SubcommandTriggeredOff", "[subcom]") {
    ...
    sub1->allow_extras(false);
    sub2->allow_extras(false);
    xcli::TriggerOff(sub1, sub2);
    args = {"sub1", "sub2"};
    CHECK_THROWS_AS(run(), xcli::ExtrasError);

    args = {"sub2", "sub1", "sub3"};
    CHECK_NOTHROW(run());
    xcli::TriggerOff(sub1, {sub3, sub4});
    CHECK_THROWS_AS(run(), xcli::ExtrasError);
    args = {"sub1", "sub2", "sub4"};
    CHECK_THROWS_AS(run(), xcli::ExtrasError);
}
...
TEST_CASE_METHOD(ManySubcommands, "SubcommandTriggeredOn", "[subcom]") {
    ...
    sub1->allow_extras(false);
    sub2->allow_extras(false);
    xcli::TriggerOn(sub1, sub2);
    args = {"sub1", "sub2"};
    CHECK_NOTHROW(run());

    args = {"sub2", "sub1", "sub4"};
    CHECK_THROWS_AS(run(), xcli::ExtrasError);
    xcli::TriggerOn(sub1, {sub3, sub4});
...
TEST_CASE_METHOD(TApp, "UnnamedSub", "[subcom]") {
    ...
    CHECK_THROWS_AS(app.get_option("--vvvv"), xcli::OptionNotFound);
    // now test in the constant context
    const auto &appC = app;
    const auto *opt3 = appC.get_option("-v");
    CHECK("--value" == opt3->get_name());
    CHECK_THROWS_AS(appC.get_option("--vvvv"), xcli::OptionNotFound);
}
...
TEST_CASE_METHOD(TApp, "FallthroughFind", "[subcom]") {
    ...
    CHECK_THROWS_AS(sub->get_option("--global"), xcli::OptionNotFound);
}
...
TEST_CASE_METHOD(TApp, "subcommand_help", "[subcom]") {
    ...
    sub1->parse_complete_callback([]() { throw xcli::CallForHelp(); });
    bool called{false};
    args = {"help"};
    try {
        run();
    } catch(const xcli::CallForHelp &) {
        called = true;
    }
...
TEST_CASE_METHOD(TApp, "DuplicateErrors", "[subcom]") {
    ...
    CHECK_THROWS_AS(sub1->add_option("-t"), xcli::OptionAlreadyAdded);
    CHECK_THROWS_AS(sub1->add_option("--t"), xcli::OptionAlreadyAdded);
    CHECK_THROWS_AS(sub1->add_option("t"), xcli::OptionAlreadyAdded);
    CHECK_THROWS_AS(sub1->add_option("-m"), xcli::OptionAlreadyAdded);
    CHECK_THROWS_AS(sub1->add_option("--m"), xcli::OptionAlreadyAdded);
    CHECK_THROWS_AS(sub1->add_option("m"), xcli::OptionAlreadyAdded);
    CHECK_THROWS_AS(sub1->add_option("quit"), xcli::OptionAlreadyAdded);
    CHECK_THROWS_AS(sub1->add_option("--quit"), xcli::OptionAlreadyAdded);
...
TEST_CASE_METHOD(TApp, "AliasErrors", "[subcom]") {
    ...
    CHECK_THROWS_AS(sub2->alias("this is a not\n a valid alias"), xcli::IncorrectConstruction);
    ...
    CHECK_THROWS_AS(app.add_subcommand("--bad_subcommand_name", "documenting the bad subcommand"),
                    xcli::IncorrectConstruction);

    CHECK_THROWS_AS(app.add_subcommand("documenting a subcommand", "sub3"), xcli::IncorrectConstruction);
    // cannot alias to an existing subcommand
    CHECK_THROWS_AS(sub2->alias("sub1"), xcli::OptionAlreadyAdded);
    CHECK_THROWS_AS(sub1->alias("sub2"), xcli::OptionAlreadyAdded);
    ...
    CHECK_THROWS_AS(sub2->alias("les2"), xcli::OptionAlreadyAdded);
    CHECK_THROWS_AS(sub1->alias("s2les2"), xcli::OptionAlreadyAdded);

    CHECK_THROWS_AS(sub2->name("sub1"), xcli::OptionAlreadyAdded);
    sub2->ignore_underscore();
    CHECK_THROWS_AS(sub2->alias("les3"), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "DuplicateErrorsPrefix", "[subcom]") {
    ...
    CHECK_THROWS_AS(app.add_subcommand("sub"), xcli::OptionAlreadyAdded);
    // cannot alias to an existing subcommand
    CHECK_THROWS_AS(sub2->alias("sub"), xcli::OptionAlreadyAdded);
    app.ignore_case();
    // this needs to be opposite of the subcommand the alias is being tested on to check for ambiguity
    sub2->ignore_case();
    CHECK_THROWS_AS(sub1->alias("SUB_"), xcli::OptionAlreadyAdded);
    app.ignore_underscore();
    sub1->ignore_underscore();
    CHECK_THROWS_AS(sub2->alias("su_bt"), xcli::OptionAlreadyAdded);
}

// test adding a subcommand via the pointer
TEST_CASE_METHOD(TApp, "ExistingSubcommandMatch", "[subcom]") {
    auto sshared = std::make_shared<xcli::App>("documenting the subcommand", "sub1");
    ...
    try {
        ...
        CHECK(!true);
    } catch(const xcli::OptionAlreadyAdded &oaa) {
...
TEST_CASE("SharedSubTests: SharedSubcommand", "[subcom]") {
    ...
    CHECK_THROWS_AS(app2.add_subcommand(xcli::App_p{}), xcli::IncorrectConstruction);
...
TEST_CASE("SharedSubTests: SharedSubIndependent", "[subcom]") {
    ...
    xcli::App_p app1 = std::make_shared<xcli::App>("test program1");
    app1->allow_extras();
    app1->add_option("-t", val2);
    auto *sub = app1->add_subcommand("", "empty name");
    sub->add_option("-v,--value", val);
...
TEST_CASE_METHOD(ManySubcommands, "getSubtests", "[subcom]") {
    xcli::App_p sub2p = app.get_subcommand_ptr(sub2);
    CHECK(sub2 == sub2p.get());
    CHECK_THROWS_AS(app.get_subcommand_ptr(nullptr), xcli::OptionNotFound);
    CHECK_THROWS_AS(app.get_subcommand(nullptr), xcli::OptionNotFound);
    xcli::App_p sub3p = app.get_subcommand_ptr(2);
...
TEST_CASE_METHOD(TApp, "subcommandEnvironmentName", "[subcom]") {
    ...
    std::string someFile;
    int sub1value{0};
    sub1->add_option("-f,--file", someFile)->envname("SOME_FILE")->required()->check(xcli::ExistingFile);
