...
TEST_CASE("app_fail") {
    ...
    } catch(const xcli::ConstructionError & /*e*/) {
        CHECK(true);
    }
}
...
TEST_CASE("fuzz_config_modifier_test1") {
    ...
    CHECK(opt3->get_multi_option_policy() == xcli::MultiOptionPolicy::Sum);
}
...
TEST_CASE("app_roundtrip_custom") {
    ...
    if(pstring_start > 0) {
        ...
        CHECK_NOTHROW(app->help("", xcli::AppFormatMode::All));
    } else {
        app->parse(parseData);
    }
...
TEST_CASE("app_roundtrip_parse_normal_fail") {
    ...
    try {
        pstring_start = fuzzdata.add_custom_options(app.get(), parseData);
    } catch(const xcli::ConstructionError & /*ce*/) {
        CHECK(true);
        return;
    }
    INFO("Failure in test case " << index);
    ...
    } catch(const xcli::HorribleError & /*he*/) {
        CHECK(false);
        return;
    } catch(const xcli::ParseError & /*e*/) {
        CHECK(true);
